...
export const round = (num: number, places = 0):
  number => Math.round(num * 10 ** places) / 10 ** places;

export const getInverseSideName = (side: string): string => (side.match(/(left)/i) ? "right" : "left");

export const getInverseSide = (text: string):
  string => text.replace(/(left)|(right)/i, getInverseSideName(text));

/**
 * @param  {IDisability[]} items
 * @return {IDisability[]}
 */
export function filterBilateralMatches(items: Array<IDisability>):
  [number[], number[]] {
  ...
  const onlyValues = (item: IDisability, idx: number, arr: IDisability[]): number => item.value;
